...
public:
	...
	Light()
	{
		...
		LightManager::AddLight(&constant);
	}
	~Light()
	{
		LightManager::RemoveLight(&constant);
	}
	void Draw()
	{
		constant.direction = Vec3(
		...
		constant.color = tcolor;
		constant.range = range;
	}

private:
	LightManager::Constant constant;
};